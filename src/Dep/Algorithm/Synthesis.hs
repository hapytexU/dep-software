...
type ProductWeight = (Int, Product')
type SumWeight = (Int, Sum')
...
extractItem
  ...
  -> Maybe (Int, ThreeValues)  -- ^ A path to a 'Leaf; with the given 'ThreeValue' together with the number of 'Zero's and 'One's in that path. If no path is found 'Nothing' is returned.
...
extractProduct
  ...
  -> Maybe ProductWeight -- A 2-tuple that contains the path to the leaf and the number of 'Zero's and 'One's in the path that measure the "weight" of the AND gate of the product.
...
extractSum
  ...
  -> Three ThreeValue -- | The 'Three' of 'ThreeValue's where we try to search for an item.
  -> Maybe SumWeight -- A 2-tuple that contains the path to the leaf and the number of 'Zero's and 'One's in the path that measure the "weight" of the OR gate of the product.
...
mergeProducts :: (Int, Product') -> (Int, Product') -> (Int, Product')
mergeProducts x@(xw, _) y@(yw, _)
  | xw <= yw = x
  | otherwise = y

minimizeProduct' :: Int -> Int -> [Bool] -> [Three Bool] -> Maybe (Int, Product')
minimizeProduct' _ _ [] ts
  | _allTrue ts = Just (0, [])
  | otherwise = Nothing
minimizeProduct' _ _ ~(x:xs) ts
  | True <- x = _pushVal'' <$> minimizeProduct' (allNstep ts DontCare) xs
  | False <- x = minimizeProduct' (allNstep ts DontCare) xs
